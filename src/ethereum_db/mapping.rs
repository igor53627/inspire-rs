...
mod tests {
    ...
    fn create_account_mapping_file(entries: &[([u8; 20], u32)]) -> NamedTempFile {
    ...
    fn create_storage_mapping_file(entries: &[([u8; 20], [u8; 32], u32)]) -> NamedTempFile {
        let mut file = NamedTempFile::new().unwrap();
    ...
    fn test_account_mapping_multiple() {
        let mut entries: Vec<([u8; 20], u32)> = (0..10u8)
            .map(|i| {
                let mut addr = [0u8; 20];
                addr[0] = i;
                (addr, i as u32 * 100)
        ...
        for (addr, expected_idx) in &entries {
            assert_eq!(mapping.lookup(addr), Some(*expected_idx as u64));
        }
    ...
    fn test_account_mapping_iter() {
        let entries: Vec<([u8; 20], u32)> = (0..5u8)
            .map(|i| {
                let mut addr = [0u8; 20];
                addr[0] = i;
                (addr, i as u32)
    ...
    fn test_storage_mapping_multiple() {
        let mut entries: Vec<([u8; 20], [u8; 32], u32)> = (0..10u8)
            .map(|i| {
                let mut addr = [0u8; 20];
                addr[0] = i;
                let mut slot = [0u8; 32];
                slot[0] = i;
                (addr, slot, i as u32 * 100)
            })
            .collect();
        ...
        for (addr, slot, expected_idx) in &entries {
            assert_eq!(mapping.lookup(addr, slot), Some(*expected_idx as u64));
        }
    }
    ...
    fn test_storage_mapping_same_address_different_slots() {
        let addr = [0x42u8; 20];
        let mut entries: Vec<([u8; 20], [u8; 32], u32)> = (0..5u8)
            .map(|i| {
                let mut slot = [0u8; 32];
                slot[0] = i;
                (addr, slot, i as u32 * 50)
            })
            .collect();
        entries.sort_by_key(|(addr, slot, _)| (*addr, *slot));
    ...
    fn test_invalid_file_size() {
        let mut file = NamedTempFile::new().unwrap();
        file.write_all(&[0u8; 25]).unwrap(); // Invalid size for account mapping
        file.flush().unwrap();
