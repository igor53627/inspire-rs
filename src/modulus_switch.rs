...
impl SwitchedPoly {
    ...
    pub fn from_poly(poly: &Poly, switched_q: u64) -> Self {
        ...
        assert!(switched_q <= u32::MAX as u64, "Switched modulus must fit in u32");
        
        let original_q = poly.modulus();
        let coeffs: Vec<u32> = poly.coeffs()
            .iter()
...
impl SwitchedSeededRgswCiphertext {
    ...
    pub fn expand(&self) -> SeededRgswCiphertext {
        ...
        SeededRgswCiphertext { rows, gadget: self.gadget.clone() }
    }
...
mod tests {
    ...
    fn test_switched_seeded_rlwe_size() {
        ...
        assert!(
            ...
            expected_reduction, actual_reduction
        );
    }
    ...
    fn test_switched_seeded_rgsw_roundtrip() {
        ...
        let gadget = crate::rgsw::GadgetVector::new(params.gadget_base, params.gadget_len, params.q);
    ...
    fn test_switched_rgsw_size_reduction() {
        ...
        println!("Reduction: {:.1}%", 100.0 * (1.0 - switched_bytes.len() as f64 / seeded_bytes.len() as f64));
